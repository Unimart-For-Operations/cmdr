...
# On DMS hosts the entire config is generated by DMS's matugen into
# ~/.config/starship.toml (starship reads a single config file, no include
# mechanism), so settings stay empty and Catppuccin remains the non-DMS default.
{ config, hostMeta ? { }, ... }:

let
  # On hosts running the DMS desktop shell, colors are owned by DMS (matugen).
  # The dank-material-shell module is only imported on DMS hosts, so its
  # `enable` flag is the signal. See dms/matugen for the template that writes
  # the full ~/.config/starship.toml on those hosts.
...
in
{
  programs.starship = {
    ...
    # On DMS hosts matugen owns ~/.config/starship.toml, so leave the HM-managed
    # settings empty (HM still sets STARSHIP_CONFIG to that path). Non-DMS hosts
    # keep the full Catppuccin prompt below.
    settings = if isDms then { } else {
